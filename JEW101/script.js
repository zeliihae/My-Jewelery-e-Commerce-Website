...
async function loadNewestProducts() {
    try {
        const response = await fetch(`http://localhost:5025/api/admin/Products?pageSize=10`);
...
        }
    } catch (error) {
        console.error("Kategori menüsü yüklenemedi:", error);
    }
}
...
        // 4. API İsteği
        const response = await fetch(`http://localhost:5025/api/Products?search=${encodeURIComponent(searchTerm)}`);
        const result = await response.json();
        
        ...
        if (response.ok && result.success) {
            ...
            const itemsWithStock = await Promise.all(
                result.data.items.map(async (item) => {
                    ...
                    try {
                        const productResponse = await fetch(`http://localhost:5025/api/admin/Products/${item.productId}`);
...
async function removeFromCart(cartItemId) {
    ...
    try {
        const response = await fetch(`http://localhost:5025/api/Cart/items/${cartItemId}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('jewelry_token')}`
            }
...
        const addrResponse = await fetch(`http://localhost:5025/api/Addresses/user/${currentUser.id || currentUser.userId}`, {
...
        }
        
        const orderResponse = await fetch(`http://localhost:5025/api/Orders`, {
            method: 'POST',
            headers: { 
                'Content-Type': 'application/json', 
...
        
        const response = await fetch(`http://localhost:5025/api/Receipt`, {
            method: 'POST',
            headers: { 
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
        
        const response = await fetch(`http://localhost:5025/api/Receipt/order/${orderId}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        
        if (response.ok) {
...
async function applyCoupon() {
    ...
    try {
        const response = await fetch(`http://localhost:5025/api/Coupons/validate`, {
            method: 'POST',
